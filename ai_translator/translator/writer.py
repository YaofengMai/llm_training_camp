...
class Writer:
    ...
    def save_translated_book(self, book: Book, output_file, file_format: str = "pdf"):
        if file_format.lower() == "pdf":
            self._save_translated_book_pdf(book, output_file)
        elif file_format.lower() == "markdown":
            self._save_translated_book_markdown(book, output_file)
        else:
            raise ValueError(f"Unsupported file format: {file_format}")

    def _save_translated_book_pdf(self, book: Book, output_file = None):
        if output_file is None:
            output_file = book.pdf_file_path.replace('.pdf', f'_translated.pdf')
        ...
        doc = SimpleDocTemplate(output_file, pagesize=pagesizes.letter)
            ...
            for content in page.contents:
                if content.status:
                    if content.content_type == ContentType.TEXT:
                        ...
                        pdf_table = Table([header_row] + data_rows)
                        pdf_table.setStyle(table_style)
                        story.append(pdf_table)
                    
                    elif content.content_type == ContentType.IMAGE:
                        #image_reader = ImageReader(content.translation)
                        img_buffer = BytesIO()
                        content.translation.save(img_buffer, format="PNG")
                        reportlab_image = Image(img_buffer)
    ...
    def _save_translated_book_markdown(self, book: Book, output_file = None):
        if output_file is None:
            output_file = book.pdf_file_path.replace('.pdf', f'_translated.md')

        LOG.info(f"pdf_file_path: {book.pdf_file_path}")
        LOG.info(f"开始翻译: {output_file}")
