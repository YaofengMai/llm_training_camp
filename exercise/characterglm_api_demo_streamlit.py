...
with st.container():
    ...
    with col1:
        ...
        st.text_area(label="角色人设提示", key="bot_prompt", on_change=lambda : st.session_state["meta"].update(bot_prompt=st.session_state["bot_prompt"]), help="角色的人设提示，不可以为空")
        #st.text_area(label="角色人设", key="bot_info", on_change=lambda : st.session_state["meta"].update(bot_info=st.session_state["bot_info"], placeholder=st.session_state["meta"]["bot_info"]), help="角色的详细人设信息，不可以为空")
        st.text_input(label="风格", key="image_style", on_change=lambda : st.session_state["meta"].update(image_style=st.session_state["image_style"]), help="模型所扮演的角色的风格，不可以为空")
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    if st.session_state["meta"]["image_style"] == "":
        image_prompt = '二次元风格。' + image_prompt.strip()
    else:
        image_prompt = st.session_state["meta"]["image_style"] + '风格。' + image_prompt.strip()
...
with st.container():
    ...
    with button_key_to_col["gen_bot_info"]:
        gen_bot_info = st.button(button_labels["gen_bot_info"], key="gen_bot_info")
...
def generate_bot_info():
    response = generate_role_appearance(st.session_state["meta"]["bot_prompt"])
    bot_info = ""
    for x in response:
        bot_info += x
    #print("bot_info:"+bot_info)
    placeholder = st.empty()
    placeholder.markdown(bot_info)
    st.session_state["meta"]["bot_info"] = bot_info 
    #st.rerun()
